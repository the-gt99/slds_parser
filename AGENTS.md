...
### Обновление цены и наличия перед экспортом 11 августа 2026

Parser commit `8353cb0` развёрнут на production с миграцией `044_export_source_freshness.sql`. Ручной preflight теперь начинается не с WordPress, а с `collect_product` в штатном collection lane с proxy lease. Source adapter явно задаёт обязательные для экспорта части; GOAT обновляет только `offers`, затем запускаются `process_product` и `preflight_product`. Прямой preview карточки и шаблона не сохраняет готовое export-control решение в обход этой цепочки.

Готовый preflight действует 15 минут. PostgreSQL не допускает в export batch результат без свежего `source_refreshed_at`, а `ExportRunner` отдельно проверяет перед HTTP-запросом, что обязательные source parts ещё свежие и internal DTO обработан после их получения. Изменение цены при подготовке не запускает параллельный автоматический export для того же target. Ошибки collection/processing переводят ожидающий preflight в `error`, а старые ready-записи без source refresh отображаются как `stale`.

Production smoke выполнен на проблемном `sourceProductId=177149`, GOAT external ID `462323`, internal product `170289`, WordPress product `2114282`. Штатная цепочка jobs `817096 → 817097 → 817098` завершилась с одной попытки: offers обновлены `2026-08-11T14:27:05.206Z`, DTO обработан после них, WordPress preflight завершён без записи. Важно: свежий GOAT US `buy_bar_data` фактически вернул два `single_in_stock` offer для размеров `8` и `9.5` по `$2693` каждый. Поэтому цена `471275 ₽` не была следствием старого snapshot или ошибки умножения: это `2693 × 175` из текущего US API. Скриншот публичного GOAT показывал доставку в Saudi Arabia и `Currently Unavailable`, то есть другой market context. Не добавлять произвольный price cap или считать такой offer отсутствующим без отдельного бизнес-правила.

После smoke новых `export_product` jobs `0`, target `slamdunk=false`, active jobs `0`, API/worker active, internal/external health `200`, свежих warning/error в journal нет. Локально и на production прошли typecheck, `351` тест и build.

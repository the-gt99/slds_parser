...
Один `Worker` использует отдельные lane-группы для discovery, collection, processing, WordPress preflight и export. Processing lanes задаются `WORKER_PROCESS_CONCURRENCY` от 1 до 16, collection lanes — `WORKER_COLLECTION_CONCURRENCY` от 1 до 16, preflight lanes — `WORKER_PREFLIGHT_CONCURRENCY` от 1 до 8, export lanes — `WORKER_EXPORT_CONCURRENCY` от 1 до 4. Эти env-значения создают первоначальную runtime-конфигурацию; затем collection, processing и preflight concurrency управляются на странице `/runtime`, сохраняются в PostgreSQL с ревизией и применяются при следующем запуске worker. Discovery остаётся последовательным. При включённом GOAT proxy pool collection lane сначала резервирует свободный session slot и только потом claim-ит `collect_product`; если свободных слотов на проверенных proxy нет, job остаётся `pending` без увеличения attempts. `JobRepository.claimNext` фильтрует типы и использует `FOR UPDATE SKIP LOCKED`. Просроченные `running` locks снова доступны после `WORKER_LOCK_TIMEOUT_MS`; число попыток увеличивается атомарно при claim.

Повторяются только `RetryableError`, пока число попыток меньше `MAX_JOB_ATTEMPTS`. Задержка растёт экспоненциально от `JOB_RETRY_BASE_MS` и ограничивается `JOB_RETRY_MAX_MS`. Для WordPress preflight/export действует отдельная длинная политика: `WORDPRESS_MAX_JOB_ATTEMPTS` (по умолчанию 12), `WORDPRESS_RETRY_BASE_MS` (5 минут) и `WORDPRESS_RETRY_MAX_MS` (1 час). Отложенный WordPress export не блокирует переход кампании к следующему товару, а окончательная ошибка товара фиксируется без остановки всей кампании. `PermanentError` и неизвестные программные ошибки сразу завершают задачу как `failed`. Terminal failure discovery дополнительно помечает активный `source_collection_run` как `failed`.
...
Пока не реализованы ETag/304 и подтверждение удаления исчезнувших товаров. Неизвестные reference-кандидаты сохраняются как штатный `classification_pending`, а не ошибка worker; export job создаётся только после полной классификации. WordPress exporter дополнительно требует перечисленные target-ом reference-типы, их `target_value_mappings` и точное сопоставление каждого размера.
...
Эти таблицы и проекция описывают legacy-контур v1, сохранённый для истории данных и контролируемого rollback. При активном Rules v2 он не участвует в принятии решений и закрыт для изменений через `rules_execution_control.freeze_legacy` и PostgreSQL-триггеры. Операционная классификация, preview и готовность товара читаются из `rules_v2`.
...
Для worker также задайте `WORKER_ID`, `WORKER_ROLE`, `WORKER_POLL_INTERVAL_MS`, `WORKER_LOCK_TIMEOUT_MS`, `WORKER_PROCESS_CONCURRENCY`, `WORKER_COLLECTION_CONCURRENCY`, `WORKER_PREFLIGHT_CONCURRENCY`, `WORKER_INVENTORY_REFRESH_CONCURRENCY`, `WORKER_INVENTORY_PREPARE_CONCURRENCY`, `WORKER_INVENTORY_SUBMIT_CONCURRENCY`, `MAX_JOB_ATTEMPTS`, `JOB_RETRY_BASE_MS` и `JOB_RETRY_MAX_MS`. `WORKER_ROLE=pipeline` запускает основной конвейер без постоянного обновления цен, `WORKER_ROLE=inventory` — только обновление цен и наличия, `all` сохраняет совместный режим для разработки и переходного запуска. Отдельную политику WordPress при необходимости можно переопределить через `WORDPRESS_MAX_JOB_ATTEMPTS`, `WORDPRESS_RETRY_BASE_MS` и `WORDPRESS_RETRY_MAX_MS`. Collection и processing concurrency допускаются от 1 до 16, preflight concurrency — от 1 до 8, остальные числовые значения должны быть положительными целыми, а базовая retry-задержка — не больше максимальной. После создания строки `runtime_worker_settings` значения из PostgreSQL являются основным runtime-источником, а env остаётся первоначальным значением.
...
- `npm run rules-v2:audit` — проверить полноту и непротиворечивость активного набора Rules v2;
- `npm run rules-v2:mode` — прочитать состояние движка либо выполнить явно заданный переход режима;
...
Обзор доступен по `/`, а рабочее место классификации Rules v2 — по `/rules-v2`. Старые адреса `/classifier` и `/classifier-config` перенаправляются на `/rules-v2` и не являются отдельным интерфейсом. Для входа используются `PARSER_ADMIN_USERNAME` и `PARSER_ADMIN_PASSWORD`; браузерная сессия подписывается `PARSER_SESSION_SECRET`, хранится в защищённой HttpOnly-cookie и действует 12 часов. Все изменения из браузера защищены CSRF-токеном. Для автоматизации API также принимает `Authorization: Bearer <PARSER_ADMIN_TOKEN>`. Токен и секрет сессии короче 32 символов, а пароль короче 12 символов не принимаются. Health endpoint остаётся публичным.

Если задан отдельный `PARSER_MCP_TOKEN` длиной не менее 32 символов, API публикует Streamable HTTP endpoint `/mcp`. Он принимает только свой Bearer token и предоставляет ограниченные инструменты активного Rules v2: workbench, карточку товара, список правил, синхронизированные WordPress-справочники, сохранённые preflight, полный read-only WordPress preview и точечную классификацию товара. Произвольного SQL, shell, export и изменения target в MCP нет. Классификация принимает только активное правило с отдельным условием `common.source.productId equals`, повторяет preview непосредственно перед записью и требует ровно один товар без конфликтов. Отдельный подтверждаемый tool может создать отсутствующий поддерживаемый term в WordPress после проверки локального справочника, но сам по себе не классифицирует и не экспортирует товар. Аудит изменений использует actor `mcp-genspark`.
...
Активные маршруты классификации Rules v2:

- `GET /api/rules-v2` — состояние движка и каталог правил;
- `GET /api/rules-v2/workbench` — товары, рассчитанные активным Rules v2, с пробелами, конфликтами и трассировкой правил;
- `POST /api/rules-v2/preview` и `POST /api/rules-v2/preview-full` — проверить влияние правила без записи;
- `GET /api/rules-v2/preview-full/:previewId` — получить результат полной фоновой проверки;
- `POST /api/rules-v2` и `PUT /api/rules-v2/:ruleId` — создать или изменить нативное правило Rules v2;
- `PUT /api/rules-v2/:ruleId/imported` — заменить импортированное правило нативной редакцией с контролем revision;
...
- `POST /api/export-control/campaigns` запускает устойчивую скользящую выгрузку с ограниченными окнами preflight, source refresh и export. Режим `safe` принимает только существующие товары с `risk=none`; `full_existing` принимает все готовые обновления, но требует завершённый `catalogRunId` и сканирует только сопоставленные товары этого снимка WordPress; `new_products` повторно проверяет и создаёт только товары, которые предыдущий preflight уже определил как отсутствующие в WordPress и готовые к созданию;
...
- `POST /api/targets/:targetId/dictionary/terms/rules-v2` — создать поддерживаемый WordPress-термин для действия Rules v2, сохранить его в локальном снимке и не обращаться к замороженным таблицам v1.
...
Постоянное обновление цен и наличия настраивается на странице `/wordpress-catalog`. Каждый цикл сначала перечитывает sitemap без полного collection, затем ставит новые и недавно изменённые товары выше остальных. Дальше отдельные очереди получают только GOAT offers, отбрасывают уже успешно применённый source fingerprint, строят `variation-patch.v3` по сохранённому снимку каталога и отправляют готовые товары пачками до 100 штук. WordPress непосредственно перед записью повторно проверяет identity, принадлежность variation товару и точный размер, поэтому отдельное повторное чтение каждого товара не требуется. Точно сопоставленный новый размер создаётся и для доступного предложения с ценой, и для недоступного без цены; во втором случае новая вариация получает `outofstock`. Неизвестные WordPress-размеры и отсутствующий size mapping записываются как пропуск. Пустой список GOAT offers блокирует изменение товара и не снимает все размеры с продажи. Расписание автоматически останавливается после ошибки товара и требует ручного продолжения. В production существующий `slds-parser-worker.service` запускается с `WORKER_ROLE=pipeline`, а `deploy/systemd/slds-parser-inventory-worker.service` обслуживает только inventory jobs. Proxy session slots координируются через PostgreSQL между процессами; `GOAT_PROXY_INVENTORY_HEADROOM` резервирует заданное количество слотов для inventory. `export_product` не резервирует proxy на весь внешний WordPress-вызов: отдельная campaign использует `refresh_export_source`, а единичный export получает короткую lease внутри source adapter.

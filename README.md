...
Пока не реализованы ETag/304, подтверждение удаления исчезнувших товаров, отдельное частое расписание обновления offers и WordPress exporter. Неизвестные reference-кандидаты сохраняются как штатный `classification_pending`, а не ошибка worker. Постановку export job определяет зарегистрированная target-specific policy: для WordPress обязательны разрешённые бренд и конечная категория, а модель обязательна при наличии семейства товара. Неизвестные цвет, материал, назначение и метки не блокируют экспорт. Версия policy входит в processing hash.

## Расширение

Новый источник подключается реализациями `SourceAdapter` и `SourceProcessor`, после чего обе явно регистрируются в `registerPipelineComponents` в `src/bootstrap.ts`. Новый target подключается реализациями `TargetExporter` и `TargetExportPolicy` с явной регистрацией там же.
...
У `reference_types` задаются разрешённые субъекты (`product`/`variant`) и кратность (`single`/`multiple`). Бренд, модель, основная категория и цвет остаются одиночными значениями товара; материалы, назначения и теги могут быть множественными, а размер относится к конкретному варианту.

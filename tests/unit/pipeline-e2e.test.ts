...
describe("pipeline end to end", () => {
  it("runs discovery, collection, processing and export through real worker and dispatcher", async () => {
    ...
    const mappings = new ReferenceMappingService(repositories.references); const collection = new CollectionRunner(repositories, unit, adapters); const processing = new ProcessingRunner(repositories, unit, processors, mappings); const exports = new ExportRunner(repositories, exporters, mappings); const dispatcher = new JobDispatcher(collection, processing, exports, repositories.sourceRuns);

...
Addon принимает только гостевой поисковый запрос с точным путём `/v3/sh-api/daga/search/goods/v1`. Host не фиксируется: актуальный iOS-клиент обращается к тому же API по IP-адресу. Addon требует точный проверочный артикул в JSON, отклоняет запросы с признаками авторизации, оставляет ровно шесть разрешённых полей и сразу отправляет их по HTTPS на парсер. Локальные файлы и база данных для профилей не используются.

При создании устройства парсер случайно выбирает артикул из реально обработанных товаров категории sneakers, у которых есть активные варианты. Выбранный артикул сохраняется в записи устройства и остаётся неизменным на время его онбординга.

...
import { registerPipelineComponents, registerProductOperations } from "../../src/bootstrap.js";
...
describe("component registries", () => {
  it("registers the GOAT pipeline components in bootstrap", () => { const adapters = new SourceAdapterRegistry(); const processors = new SourceProcessorRegistry(); const operations = new ProductOperationRegistry(); const exporters = new TargetExporterRegistry(); registerPipelineComponents({ adapters, processors, operations, exporters }, { PARSER_PUBLIC_BASE_URL: "https://parser.example/images", SHOE_HEIGHT_API_URL: "http://classifier.example/predict" }); expect(adapters.get("goat").code).toBe("goat"); expect(processors.get("goat").sourceCode).toBe("goat"); expect(operations.listForSource("goat").map((item) => item.code)).toEqual(["normalize-product", "translate-content", "download-images", "detect-shoe-height", "convert-images-to-webp", "publish-images", "validate-processed-product"]); });
  it("does not invalidate operation output when only image transport concurrency changes", () => {
    const slow = new ProductOperationRegistry();
    const fast = new ProductOperationRegistry();
    registerProductOperations(slow, { PARSER_PUBLIC_BASE_URL: "https://parser.example/images", GOAT_IMAGE_DOWNLOAD_CONCURRENCY: "2" });
    registerProductOperations(fast, { PARSER_PUBLIC_BASE_URL: "https://parser.example/images", GOAT_IMAGE_DOWNLOAD_CONCURRENCY: "8" });

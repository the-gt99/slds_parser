...
import { ProductOperationRegistry, SourceAdapterRegistry, SourceProcessorRegistry, TargetExporterRegistry, TargetExportPolicyRegistry } from "../../src/core/registry/index.js";
...
describe("pipeline end to end", () => {
  it("runs discovery, collection, processing and export through real worker and dispatcher", async () => {
    ...
    const processors = new SourceProcessorRegistry(); const processor: SourceProcessor = { sourceCode: "fake", version: "1", process: vi.fn().mockImplementation(async ({ sourceProduct }) => validProduct(sourceProduct.id)) }; processors.register(processor);

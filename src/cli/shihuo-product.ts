...
try {
  ...
  if (mode === "resolve") {
    ...
    console.log(JSON.stringify({ status: result.status, sourceProductId: result.sourceProductId, article: result.article,
      goodsId: result.goodsId ?? null, styleId: result.styleId ?? null, card: result.card ?? null }, null, 2));
  } else if (mode === "fetch-card") {
    console.log(JSON.stringify(await application.shihuoResolver.fetchResolvedProductCard({ sourceProductId: id }), null, 2));

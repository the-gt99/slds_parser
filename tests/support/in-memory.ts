...
export function createMemoryRepositories(store: MemoryStore): TransactionRepositories {
  return {
    classifications: {
      ...
      getActiveRuleSetRevision: async (sourceId) => JSON.stringify(store.classificationRules
        .filter((rule) => rule.sourceId === sourceId)
        .map((rule) => [rule.id, rule.revision, rule.referenceValueId])),
      listAllActiveRules: async (sourceId) => store.classificationRules.filter((rule) => rule.sourceId === sourceId),
    ...
    sourceProducts: {
      ...
      upsertDiscovered: async (input: UpsertDiscoveredSourceProductInput) => { const previous = [...store.products.values()].find((item) => item.sourceId === input.sourceId && item.sourceKey === input.sourceKey); const id = previous?.id ?? store.id(); const record: SourceProductRecord = { id, sourceId: input.sourceId, sourceKey: input.sourceKey, externalId: input.externalId ?? previous?.externalId ?? null, slug: input.slug ?? previous?.slug ?? null, url: input.url ?? previous?.url ?? null, discoveryMetadata: input.discoveryMetadata, status: input.status, firstSeenAt: previous?.firstSeenAt ?? input.seenAt, lastSeenAt: input.seenAt, lastSeenRunId: input.runId, createdAt: previous?.createdAt ?? timestamp, updatedAt: timestamp }; store.products.set(id, record); return { product: record, created: previous === undefined, discoveryChanged: previous === undefined || JSON.stringify(previous.discoveryMetadata) !== JSON.stringify(input.discoveryMetadata) }; },
...
export class MemoryJobRepository implements JobRepository {
  ...
  async claimNext(workerId: string, _lockTimeoutMs: number, jobTypes?: readonly JobRecord["jobType"][]): Promise<JobRecord | null> { const classificationApplicationActive = [...this.store.jobs.values()].some((item) => item.jobType === "apply_target_classification_suggestion" && ["pending", "running", "retry"].includes(item.status)); const job = [...this.store.jobs.values()].find((j) => ["pending", "retry"].includes(j.status) && (jobTypes === undefined || jobTypes.includes(j.jobType)) && (j.jobType !== "reclassify_product" || !classificationApplicationActive)); if (!job) return null; const claimed: JobRecord = { ...job, status: "running", attempts: job.attempts + 1, lockedAt: timestamp, lockedBy: workerId }; this.store.jobs.set(job.id, claimed); return claimed; }

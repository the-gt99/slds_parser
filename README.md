...
Если задан отдельный `PARSER_MCP_TOKEN` длиной не менее 32 символов, API публикует Streamable HTTP endpoint `/mcp`. Он принимает только свой Bearer token и предоставляет ограниченные инструменты активного Rules v2: workbench, карточку товара, список правил, target-справочники, preview и точечную классификацию товара. Произвольного SQL, shell, export, изменения target и создания внешних terms в MCP нет. Write-инструмент принимает только активное правило с отдельным условием `common.source.productId equals`, повторяет preview непосредственно перед записью и требует ровно один товар без конфликтов. Аудит таких изменений использует actor `mcp-genspark`.

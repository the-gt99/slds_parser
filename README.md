...
- `POST /api/export-control/preflights` — поставить выбранные либо следующие устаревшие товары в цепочку свежих source offers, обработки и WordPress preflight;
...
Отдельный интерфейс `/export-control` показывает только сохранённый компактный read model: создание или обновление WordPress, поля и taxonomy с изменениями, добавление и снятие меток, изображения, вариации, блокировки и последний export job. Открытие страницы не собирает payload заново и не обращается к WordPress. Постановка проверки сначала обновляет определённые source adapter-ом части цены и наличия в обычном collection lane, затем обрабатывает DTO и запускает WordPress preflight. Для GOAT повторно запрашивается только `offers`; карточка и изображения у GOAT повторно не запрашиваются. Несколько независимых preflight lanes могут выполнять read-only WordPress-проверки параллельно; список использует keyset pagination и индексы вместо `OFFSET` и `COUNT(*)`. Изменение обработанного товара или target-конфигурации делает результат устаревшим, а готовый результат с source offers старше 15 минут автоматически считается устаревшим.

Ручной экспорт принимает только свежие готовые результаты. При подтверждении payload hash и ожидаемая WordPress identity замораживаются вместе с batch items и export jobs в одной транзакции. Перед фактической записью exporter проверяет, что обязательные source parts ещё свежие и DTO обработан после их получения, повторно строит payload и делает свежий WordPress lookup; изменение payload, найденного товара или способа совпадения останавливает запись. После любой попытки экспорта сохранённый preflight помечается устаревшим, чтобы прежний diff нельзя было повторно принять за актуальный.

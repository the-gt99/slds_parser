...
try {
  ...
  for (const id of ids) {
    ...
    if (!equal) mismatches++;
    const payload = outcomes[1]!.payload;
    if (equal && payload !== undefined) {
      payloads++;
      ...
      try {
        ...
        preflightPassed++;
        results.push({ id, equal, payloadHash: (payload as { payload_hash?: string }).payload_hash,
          preflight: { willCreate: preflight.willCreate, externalId: preflight.externalId, matchedBy: preflight.matchedBy } });
      } catch (error) {
        preflightBlocked++;
        results.push({ id, equal, preflightError: error instanceof Error ? error.message : String(error) });
      }
    } else results.push({ id, equal, outcomes });
  }
  await client.query("COMMIT");
  console.info(JSON.stringify({ revision: snapshot.revision, checked: ids.length, payloads, mismatches, preflightPassed, preflightBlocked, writes: false, results }, null, 2));
  if (mismatches > 0 || payloads === 0 || preflightBlocked > 0) process.exitCode = 1;

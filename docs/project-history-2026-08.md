# SLDS Parser: архив рабочего контекста на 18 августа 2026 года
...
GOAT `story` и обычное `description` сохраняются и переводятся как разные поля. SourceProcessor не подменяет одно другим. WordPress exporter выбирает текст для нового описания в порядке `translated story → translated description`, но при обновлении любое непустое существующее описание WordPress имеет абсолютный приоритет и сохраняется целиком.
...
2. **Несколько брендов и моделей.** WordPress target и exporter версии `1.9.0` допускают несколько `pa_brand` и `pa_model`; дополнительные terms можно добавлять projections или target assignment rules. GOAT по-прежнему даёт один основной source-бренд и одну модель-кандидат. Размерная сетка выбирается только по основному разрешённому source-бренду, поэтому дополнительный бренд не делает конвертацию неоднозначной. Для коллабораций всё ещё нужны проверенные точные условия: не сохранять широкие правила по одному семейству или слову.
...
- multiple-cardinality `pa_brand` и `pa_model` реализована end-to-end; для коллабораций всё ещё нужно определить воспроизводимые точные source/model-условия дополнительных назначений. Реальные GOAT payload для Valentino Garavani x Vans содержат только основной `brandName: Vans`;
...
Переводчик проверен относительно v1 и production-данных. V1 использовал неофициальный Google Translate через `stichoza/google-translate-php`, а суперстарый `import.slamdunk.shop` — платный DeepL API v2 с кешем в таблице `translations`. Текущий parser поддерживает оба provider: `PARSER_TRANSLATION_PROVIDER=google|deepl`, для DeepL нужен `PARSER_DEEPL_API_KEY`; автоматического fallback нет. Смена provider входит в configuration fingerprint операции. Для массовой смены есть отдельный `retranslate_product`: он обновляет только `translatedContent` сохранённого DTO, не вызывает SourceProcessor, GOAT, media, shoe-height, классификацию или export. Успешные ответы постоянно кешируются в `translation_cache`; provider/version и языки входят в ключ. Массовая CLI сначала показывает dry-run объёма, apply ставит идемпотентные jobs. Системные ошибки Google были в пассивных обувных атрибутах: `Knit -> Вязать`, `Synthetic -> Синтетический`, `Navy -> Флот`, `Racer Blue -> Гонщик Синий`. Операция перевода поднята до `1.1.0`: добавлен проверенный словарь материалов и частых расцветок, удаляются zero-width/неразрывные символы, одинаковые `description` и `story` переводятся одним запросом. На `76399` теперь получены `Антрацит / Синий / Черный`, `Синтетика` и чистый текст без невидимых пробелов.
...
### Актуализация цены и наличия внутри экспорта 11 августа 2026

Первоначальная реализация commit `8353cb0` с 15-минутным TTL заменена parser commit `1b21642` и миграцией `045_live_export_refresh.sql`. Колонка `source_refreshed_at` и freshness-ограничения удаляются. Ручной preflight снова выполняет только read-only проверку WordPress по сохранённому DTO; он не запускает `collect_product` или `process_product`.

В `/runtime` есть настройка `refreshSourceBeforeExport`, по умолчанию включённая. После перезапуска worker она применяется вместе с concurrency-настройками. Когда `export_product` реально взят export lane, worker резервирует GOAT proxy lease, один раз запрашивает заданные adapter-ом source parts и продолжает ту же задачу. Для GOAT это только `buy_bar_data`/`offers`; карточка товара и изображения не запрашиваются. Из свежих offers отдельно извлекаются цена, наличие и размеры, сохранённый offers part обновляется, но полные `ProductOperation`, перевод, изображения, классификация и повторный preflight не запускаются. Новая job не создаётся, поэтому товар не переносится в конец очереди из-за изменения цены.

WordPress payload подставляет live-вариации непосредственно перед upsert. Одобренный preflight по-прежнему замораживает статические поля: изменение title, taxonomy, изображений, шаблонов или mappings блокирует запись. Отличия только в live-вариациях не считаются нарушением одобрения; перед записью всё равно повторно проверяется WordPress identity. При выключенной настройке exporter использует варианты из сохранённого internal DTO как раньше.
...
## Фоновое применение сохранённых назначений WordPress 12 августа 2026

Массовое применение безопасных предложений на вкладке «По товарам WordPress» не должно выполняться пакетным циклом из браузера. Один HTTP-запрос атомарно создаёт отдельный `apply_target_classification_suggestion` job для каждого подходящего предложения и переводит его в `queued`; выделенная одиночная worker lane последовательно создаёт mapping/rule и ставит затронутые товары на `process_product`. Интерфейс показывает отдельный счётчик «В очереди» и опрашивает состояние, но закрытие вкладки не прерывает работу. При окончательной ошибке предложение возвращается в `ready` с сохранённым текстом ошибки.

Этот процесс изменяет только конфигурацию классификатора и PostgreSQL jobs. Само применение предложений не записывает товары в WordPress и не создаёт `export_product`; target `slamdunk` должен оставаться выключенным до отдельного подтверждённого экспорта.

Parser commit `4ba33b3` развёрнут на production, миграция `050` применена. Массовый API-запрос завершился с HTTP `200` за `4.1 с` и поставил в очередь все оставшиеся `9895` безопасных предложений импорта `#1`; `172` конфликта остались на ручную проверку. Контрольная проверка показала последовательный переход jobs в `completed`, отсутствие `retry`/`failed`, active API/worker, health `200`, target `slamdunk=false` и `0` активных export jobs. Полное применение правил и последующая переобработка товаров продолжаются в фоне.
...
## Production deploy нескольких брендов и моделей 12 августа 2026

Parser commits `1e82f7b` и `cd9a7e4` развёрнуты на production. WordPress provider и exporter принимают несколько `pa_brand` и `pa_model`; exporter version поднята до `1.9.0`. Дополнительные target terms не меняют основной source-бренд, используемый для брендовой конвертации размеров. В редактор target assignment rules добавлены условия по исходному бренду, исходной модели и её brand/family context.

Локально прошли typecheck, `378` тестов и build. На production прошли typecheck, `377` тестов до UI-дополнения, отдельный новый matcher test, build и migrations (`No pending migrations`). API и worker active, health `200`, target `slamdunk=false`, production worktree чистый.

Read-only проверка `sourceProductId=141235`, GOAT `1337456`, WordPress `1291572` подтвердила основной `adidas`, существующий WordPress `Clarks` и исходную модель `Ronnie Fieg x Clarks x adidas 8th Street Samba 'Kithmas Collection - White Black'`. Модель пока unresolved, поэтому товар остаётся `classification_pending`; preview assignment rules считает только `classified` products и для черновика точного дополнительного `Clarks` вернул `0`. Правило не сохранялось, WordPress write и export jobs не выполнялись. Сначала нужно разрешить модель либо принять отдельное проверенное правило для набора точных source model values, затем повторить preview и проверить `pa_brand=[adidas, Clarks]`.

### Временный пропуск размеров без WordPress mapping 13 августа 2026

WordPress target поддерживает явный флаг `ignoreUnmappedSizeVariants`. При включённом флаге exporter не блокирует весь товар из-за отдельного варианта без точного `sizeMappings`, а исключает только этот вариант из payload. Пропуск не должен быть скрытым: preview возвращает полный список `ignoredSizeVariants` с исходным размером, системой, аудиторией, ценой и причиной. Если сопоставленных вариантов не осталось, экспорт по-прежнему блокируется. Это временная политика до добавления недостающих WordPress-размеров; нельзя подбирать ближайший термин или автоматически подменять `Y` на `K`.

Для `sourceProductId=129236`, GOAT `913979`, WordPress `2624382` сохранённый DTO содержит шесть доступных офферов: `3=$117`, `4.5=$1178`, `5.5=$96`, `11=$103`, `12.5=$2463`, `13.5=$2463`. WordPress содержит четыре старые вариации: `3Y`, `4.5Y`, `5.5Y`, `11Y`; target не имеет терминов и mappings для `12.5Y` и `13.5Y`. Новый parser не разворачивает полный `sizeRange`: он использует только реальные строки `buy_bar_data`.

Отдельно предложено GOAT-правило ценовых выбросов `×5`: в примере нормальная нижняя группа `$96/$103/$117`, после которой `$1178/$2463/$2463` должны быть исключены. Правило пока не активировано, потому что не определён воспроизводимый способ выбора базовой группы. На текущей базе вариант `цена > minimum × 5` затрагивает `67 703` товара и `193 366` вариантов; вариант «первый разрыв между соседними отсортированными ценами > ×5» — `43 700` товаров и `120 198` вариантов. Перед реализацией нужно выбрать точную формулу, проверить репрезентативную выборку и показывать отброшенные цены в preview так же явно, как пропущенные размеры. Не применять абсолютный price cap.

Parser commit `2fa62ff` развёрнут на production, `ignoreUnmappedSizeVariants=true`, target по-прежнему выключен. Реальный read-only preflight товара `129236` стал ready: `6` исходных вариантов, `4` после merge, `4` сейчас; `12.5Y` и `13.5Y` явно перечислены как пропущенные, деактиваций нет. Ценовое правило не включено, поэтому `4.5Y=$1178` пока остаётся в payload как `206150 ₽`; до согласования формулы этот diff нельзя экспортировать. Активных export jobs `0`, API/worker active, warning/error после deploy нет.

## Каталог WordPress и price-only автопрогон 14 августа 2026

Административный каталог переработан и развёрнут до commit `c8a3d2f`, migration `057`. Список использует отдельную компактную `wordpress_catalog_item_read_models`: тяжёлые `snapshot.payload` и `audit_result` не входят в постраничный ответ, полный локальный diff загружается отдельным запросом только для одного товара. Поиск охватывает title, WordPress/parser/internal/GOAT ID и SKU; доступны фильтры аудита, риска, операции, типов изменений и состояния price-only обработки. Меню всех административных страниц формируется единым `public/admin-shell.js`.

Полный аудит остаётся локальным сравнением сохранённого WordPress snapshot с сохранённым parser DTO и не обращается к WordPress при просмотре. Price-only автопрогон перед записью обновляет GOAT offers, сопоставляет только уже существующие WordPress-вариации и не создаёт новые размеры. После deployment run `1` продолжил выполняться с окном `5000`, без сброса очереди и без ошибок; на контрольной точке было `20 796` completed, `4 611` безопасно skipped, `0` failed и `4 993` активных задач.

## Сохранённый preflight, source facts и визуальный diff 14 августа 2026

Parser commits `cf591b3`, `02a1d6e` и `947baca` развёрнуты на production. Карточка товара автоматически показывает последний сохранённый WordPress preflight через локальный GET без запроса к WordPress; явная кнопка «Обновить с WordPress» выполняет read-only запрос и сохраняет новый результат. Каталог WordPress открывает полный diff в визуальном режиме с двумя карточками «сейчас / после merge» и оставляет прежний технический режим отдельной вкладкой. Для старых аудитов будущие изображения восстанавливаются только по сохранённым content hash из текущего internal DTO, поэтому не подменяются произвольной картинкой.

Parser commits `6a63f7b`, `591204f`, `0ad61f0` и `cd37c83` расширили визуальный modal diff до полного preflight: после сводных карточек доступны таксономии, основные поля с безопасно отрендеренными описаниями, вариации и размеры, попарное сравнение изображений. Окно имеет собственную вертикальную прокрутку, техническая вкладка сохранена. Старые аудиты без построчных `variations.items` восстанавливают таблицу из сохранённого WordPress snapshot и локального variation payload, а состав размеров берут из аудита; подписи размеров восстанавливаются из WordPress `term_slug` и target `sizeMappings`. Production smoke на `sourceProductId=11485` подтвердил 27 строк `US ...`, восемь пар изображений и прокрутку `805 / 3891 px`; ошибок браузерной консоли нет. Локально и на production прошли typecheck, build и 432 теста.

Универсальный DTO получил нейтральный `sourceFacts`; GOAT processor `3.1.0` сохраняет там `designer`, но не создаёт для дизайнеров reference candidates и не расширяет очередь классификатора. Target assignment rules поддерживают точное условие `product.fact.designer`. Production-товар `sourceProductId=13791` точечно переобработан штатной job от пользователя `slds-parser`: `sourceFacts.designer=Wilson Smith`, классификация complete. Read-only preview правила с существующим WordPress-тегом Wilson Smith вернул ровно этот товар; правило не сохранялось и WordPress write не выполнялся.
...
На сохранённом catalog run `4` было `3 780` ready-аудитов со снятием `pa_model`. Parser commit `6d47948` добавил штатный локальный `rebuildAudits` с опциональным фильтром сохранённого change flag; он не скачивает каталог заново и не пишет в WordPress. Первый пересчёт убрал `1 891` старое снятие. Дополнительная проверка выявила варианты написания `Hi/High` и необязательное слово `Retro`; commit `9569ac4` нормализовал их, не ослабляя обязательность токена `High`. Повторный пересчёт оставшихся `1 889` строк четырьмя jobs убрал ещё `31`, осталось `1 858` неподтверждённых либо корректных изменений, pending и ошибок `0`. Из прежних `Air Jordan 1 Retro High -> Air Jordan 1` осталось только 13 товаров без `High/Hi` в source-модели (`Anodized`, `Pinnacle`, обычный `Retro` и SB), поэтому точный high-термин для них намеренно не сохраняется. Сохранённые аудиты `sourceProductId=218722` и `63596` больше не содержат diff или change flag по `pa_model`: в обоих `before` и `after` совпадают с `Air Jordan 1 Retro High` term `14777` и `Nike SB Dunk High` term `14746`. После deploy настройка включена для target `slamdunk` при сохранении `preserveExistingBrandTerms=true` и `preserveExistingTagTerms=true`; сам target остался `enabled=false`.

Попытка создать новый full catalog run `5` выявила тяжёлый первый `savePage`: транзакция более шести минут читала диск и не сохранила ни одной строки. Пустой run `5` и его единственная sync job удалены после отмены точного PostgreSQL backend; UI снова показывает завершённый run `4` с `220 029` товарами. До отдельной оптимизации этого SQL новый полный catalog run не запускать. Variation sync и export jobs не создавались.
...
- Не экспортировать товары с пустыми offers. Товары с несколькими брендами или моделями экспортировать только после точного дополнительного назначения и проверки полного taxonomy diff.

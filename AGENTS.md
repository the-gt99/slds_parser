...
- бренд;
- полное название как кандидат модели, с брендом и семейством в контексте;
- категория;
- цвет;
- материал;
- теги, если они присутствуют.
...
Добавлять `activity`, `shoe_height` или настоящий сезон в очередь можно только после появления воспроизводимого правила извлечения из данных источника. Сам механизм и WordPress-справочники для них уже готовы.
...
- «Путь товара» сейчас смешивает всю историю jobs, текущие parts и операции; из-за этого старые smoke-запуски выглядят как повторяющийся сбор. Нужно группировать по запуску/попытке и по умолчанию показывать последний;
- сырое поле GOAT `season` нужно подписать «Год коллекции GOAT» либо скрыть из основного блока;
- полезно добавить общий поиск/список товаров и явную кнопку безопасной переобработки, но это не должно опередить exporter.

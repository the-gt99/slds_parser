...
На сохранённом catalog run `4` было `3 780` ready-аудитов со снятием `pa_model`. Parser commit `6d47948` добавил штатный локальный `rebuildAudits` с опциональным фильтром сохранённого change flag; он не скачивает каталог заново и не пишет в WordPress. Все `3 780` строк с `taxonomy_removed:pa_model` пересчитаны восемью jobs по 500 товаров: `1 891` старое снятие исчезло, `1 889` осталось как неподтверждённое либо корректное изменение, pending `0`. Сохранённые аудиты `sourceProductId=218722` и `63596` больше не содержат diff или change flag по `pa_model`; real read-only preflight возвращают соответственно только `Air Jordan 1 Retro High` term `14777` и `Nike SB Dunk High` term `14746`. После deploy настройка включена для target `slamdunk` при сохранении `preserveExistingBrandTerms=true` и `preserveExistingTagTerms=true`; сам target остался `enabled=false`.

...
function parseVariants(product: JsonObject, offersPayload: JsonObject): {
  ...
  return { productId, countryCode, offersCount: offersPayload.offers.length, variants };
}
...
export class GoatSourceProcessor implements SourceProcessor {
  ...
  async processExportRefresh(context: ProcessingContext) {
    ...
    return { variants: parseVariants(object(productPart.parsedPayload, "product"), object(offersPart.parsedPayload, "offers")).variants };
  }
  ...
  async process(context: ProcessingContext): Promise<UniversalProductDTO> {
    ...
    const { productId, countryCode, offersCount, variants } = parseVariants(product, offersPayload);
    ...
    return { sourceProductId: context.sourceProduct.id, title, description, sku: text(product.sku),
      ...
      metadata: { source: "goat", productId, slug: product.slug ?? context.sourceProduct.slug ?? null, route: route || null, countryCode,
        offersCount, activeVariantCount: variants.length,

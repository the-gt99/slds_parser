...
import { ProductOperationRegistry, SourceAdapterRegistry, SourceProcessorRegistry, TargetExporterRegistry, TargetExportPolicyRegistry } from "./core/registry/index.js";
...
import { GoatImageDownloader, GoatSourceAdapter, GoatSourceProcessor, WordPressExportPolicy, type GoatHttpEnvironment } from "./integrations/index.js";
...
export function registerPipelineComponents(registries: {
  ...
  readonly exportPolicies: TargetExportPolicyRegistry;
  ...
  registries.exportPolicies.register(new WordPressExportPolicy());
...
export function createApplication(environment: ApplicationEnvironment = process.env) {
  ...
  const exportPolicies = new TargetExportPolicyRegistry();
  registerPipelineComponents({ adapters, processors, operations, exporters, exportPolicies }, environment);
  ...
  const processingRunner = new ProcessingRunner(repositories, unitOfWork, processors, operationPipeline, classifier, exportPolicies);
  ...
  return { pool, repositories, unitOfWork, adapters, processors, operations, exporters, exportPolicies, classifier, targetMappings, collectionRunner, operationPipeline, processingRunner,

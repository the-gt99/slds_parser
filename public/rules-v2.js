...
const state = { session: null, schema: null, targets: [], overview: null, selected: null, searchTimer: null, workbenchOffset: 0, workbench: null };
...
function renderWorkbench() {
  ...
  byId("workbench-summary").replaceChildren(
    ...
    node("small", "muted", `Проверено на странице: ${number(data?.scannedCount)}`),
  );
  ...
  byId("workbench-items").replaceChildren(...items.map((item) => {
    ...
    card.append(heading, fields, facts, issues, details, candidates);
    return card;
  }));
...
async function loadWorkbench() {
  byId("workbench-loading").hidden = false; byId("workbench-items").replaceChildren(); byId("workbench-empty").hidden = true;
  try {
...
async function preview() { try { const data = await api("/api/rules-v2/preview", { method: "POST", body: body() }); const result = data.preview; const box = byId("preview-result"); box.replaceChildren(node("strong", "", `${result.scope === "sample" ? `Проверено ${number(result.examined)} последних товаров. Совпадений` : "Совпало товаров"}: ${number(result.productCount)}`), node("p", "muted", "Записей в WordPress: 0. Preview использует тот же V2-исполнитель.")); if (result.conflicts?.length) { const list = node("ul", "preview-blockers"); result.conflicts.forEach((item) => { const row = node("li"); const link = node("a", "", `Товар #${item.sourceProductId}`); link.href = `/products/${encodeURIComponent(item.sourceProductId)}`; row.append(link, document.createTextNode(`: ${item.message}`)); list.append(row); }); box.append(node("h4", "", `Конфликты: ${result.conflicts.length}`), list); } if (result.examples?.length) { const examples = node("details", "workbench-trace"); examples.append(node("summary", "", `Примеры затронутых товаров: ${result.examples.length}`)); result.examples.forEach((item) => { const link = node("a", "", item.title || `Товар #${item.sourceProductId}`); link.href = `/products/${encodeURIComponent(item.sourceProductId)}`; const row = node("p"); row.append(link); examples.append(row); }); box.append(examples); } box.hidden = false; } catch (cause) { error(cause.message); } }
async function save() { try { const payload = body(); const url = state.selected ? `/api/rules-v2/${state.selected.id}${state.selected.originKind === "native" ? "" : "/imported"}` : "/api/rules-v2"; await api(url, { method: state.selected ? "PUT" : "POST", body: payload }); await loadRules(); reset(); await loadWorkbench(); toast("Правило сохранено. Результирующие DTO пересчитаны."); } catch (cause) { error(cause.message); } }
